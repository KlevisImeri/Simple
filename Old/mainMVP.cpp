...
void onDisplay() {
  ...
  mat4 MVP = TranslateMatrix(vec3(sin(time / division), 0.0, 0.0)) *
             ScaleMatrix(vec3(0.5, 0.5, 0.5)) *
             RotationMatrix(time / (M_PI * division), vec3(0.0, 0.0, 1.0));
...
void onMouseMotion(int pX,
                   int pY) { // pX, pY are the pixel coordinates of the cursor
                             // in the coordinate system of the operation system
