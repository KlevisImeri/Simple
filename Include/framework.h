...
class Texture {
  //---------------------------
  std::vector<vec4> load(std::string pathname, bool transparent, int &width,
                         int &height) {
...
public:
  ...
  void create(int width, int height, const std::vector<vec4> &image,
              int sampling = GL_LINEAR) {
    if (textureId == 0)
...
public:
  ...
  bool create(const char *const vertexShaderSource,
    ...
    if (geometryShaderSource != nullptr) {
      ...
      glShaderSource(geometryShader, 1, (const GLchar **)&geometryShaderSource,
                     NULL);
    ...
    glBindFragDataLocation(shaderProgramId, 0,
                           fragmentShaderOutputName); // this output goes to the
                                                      // frame buffer memory
  ...
  void setUniform(const Texture &texture, const std::string &samplerName,
